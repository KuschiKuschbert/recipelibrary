...
| Area | Purpose |
|------|---------|
| [index.html](index.html) | Main library UI — loads **`alpha_catalog/`** merged JSON parts, recipe detail from `recipe_detail/`, merges **user recipes** from the browser; **+ Kitchen book** and pills for custom books |
...
| [leichhardt.html](leichhardt.html) | **Leichhardt → Cook Off**: barramundi and eye fillet trial recipes, component ingredients, methods, original plating visuals and one-page PDF. Source: [`leichhardt_data/cook_off.json`](leichhardt_data/cook_off.json); render with `python3 scripts/build_leichhardt_page.py` and verify with `--check`. |
...
- **`alpha_catalog`** loads **8** merged shards with **bounded concurrency** (8 parallel fetches on [index.html](index.html) and [pantry.html](pantry.html)). Building **filter facet sets**, **`_searchHay`**, and the **`recipeIndex`** map is **chunked** with `requestAnimationFrame` yields so the main thread can still paint during load.
...
`alpha/index.json` defines **letter → stem** rules (which Unicode first letters map to which bucket stem). The rebuild script assigns each recipe to a **logical** letter bucket from the **first letter of `name`**, then **merges** those buckets into **eight** physical files `catalog_01.json` … `catalog_08.json` for fewer HTTP requests. You do **not** need the large `alpha/*.json` full-recipe copies for the site to work (they are optional archives; see **Repo size** below).
...
For a quick pre-commit gate, run:

```bash
python3 scripts/ship_check.py
```
...
- **`scraped_raw/`** — import/scrape intermediates.
- **`kitchen_library_*.json`** — legacy full-library dumps.
- **`alpha/*.json`** except **`alpha/index.json`** — full-recipe letter archives (the site uses `alpha_catalog/` + `recipe_detail/`).

Removing them from git does not shrink **historical** clone size until you rewrite history (e.g. BFG); it stops shipping them in **new** commits.
