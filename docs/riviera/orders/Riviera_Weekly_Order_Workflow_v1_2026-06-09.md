...
- **Item** — qty — status — allocated to event
...
RISK / CHECK BEFORE ORDERING
- Risk item — action required
```

---

## 12. Supplier-ready message rule

When writing supplier messages, keep them clean.

Use:

```text
Subject: Riviera Weekly Order — Delivery [Date]
...
Do not include event splits in supplier email unless supplier needs them.
...
```text
□ Every event in week included
□ Product formats resolved
□ 9% buffer applied only where relevant
□ Event split visible for combined ingredients
□ Already in-house checked
□ Already ordered checked
□ Doblo's delivery date shown
□ Bidfood order date shown
□ Woolworths/Coles top-ups separated
□ Specialty supplier items separated
□ Missing pack sizes flagged
□ Over-order risks flagged
□ Allergen-sensitive items checked against supplier source where available
□ No invented prices
□ No hidden missing items
```
