...
The Kuschi kitchen library repo uses: `claude_index/*.json` + `recipe_detail/detail_*.json`, index `_searchHay` + filter memo, `pantry_data/shard_hay_index.json` + `scripts/build_pantry_shard_hay_index.py`, external Riviera builtins + aroma preload/prime.
