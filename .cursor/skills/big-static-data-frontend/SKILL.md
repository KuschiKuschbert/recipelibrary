...
The Kuschi kitchen library repo uses: **`recipe_detail/` as SSOT** for full recipes; **`python3 scripts/rebuild_catalog_from_detail.py`** regenerates `claude_index/`, **eight merged** `alpha_catalog/catalog_XX.json` + `manifest.json`, and `pantry_data/shard_hay_index.json`. The browser loads `alpha_catalog/` only; modals use `recipe_detail/detail_{L}_{bucket}.json` (lazy fetch; 64 FNV-1a buckets) with **`_detailLetter`** on index rows when the display `name`’s first letter differs from the detail filename letter. Index `_searchHay` + filter memo; external Riviera builtins + aroma preload/prime.
