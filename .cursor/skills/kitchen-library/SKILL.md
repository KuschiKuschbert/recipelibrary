...
- **Riviera order list UI:** [riviera.html](riviera.html) — ingredients grouped **by storage zone** (not by recipe); per-line order qty, zone, include checkbox; “Add to master & list”; copy plain text, **Copy order data JSON** (`exportOrderBundle`), **Copy master ingredients JSON** (`exportMaster`). Recipe lines merge on **`canonicalOrderMergeKey(item)` only** (same name across zones becomes one row; listed under the majority zone; recipe hint prefixes by zone when amounts split). Canonical key lives in [assets/user-recipes.js](assets/user-recipes.js) (punctuation/`&`/dash variants, Kewpie/dill synonyms). Manual **order extras** with the same canonical name fold in if their zone appears in that merged row (sub-row for manual qty/remove; changing the parent row zone updates folded extras).
