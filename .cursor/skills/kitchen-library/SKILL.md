...
- **Riviera order list UI:** [riviera.html](riviera.html) — ingredients grouped **by storage zone** (not by recipe); per-line order qty, zone, include checkbox; “Add to master & list”; copy plain text, **Copy order data JSON** (`exportOrderBundle`), **Copy master ingredients JSON** (`exportMaster`). Recipe lines merge on `zone + canonicalOrderMergeKey(item)` ([assets/user-recipes.js](assets/user-recipes.js)) so punctuation/`&`/dash variants and a few synonyms (e.g. Kewpie lines, dill) collapse. Manual **order extras** with the same canonical name and zone fold into that recipe row (sub-row for manual qty/remove; changing the parent row zone updates folded extras).
