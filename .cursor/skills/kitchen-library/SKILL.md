...
- **Riviera stocktake catalog:** [assets/stocktake-data.js](assets/stocktake-data.js) defines `RIVIERA_STOCKTAKE_CATALOG` (~365 built-in rows: zone, category, default qty/brand/UOM). [assets/stocktake-list.js](assets/stocktake-list.js) renders them after recipe/order rows with sub-category headers; row ids `builtin:<id>` in `kuschi_riviera_stocktake_v1` `lines` override defaults. **Clear counted** removes `builtin:*` keys so defaults return. Regenerate data with `python3 scripts/generate_riviera_stocktake_data.py`.
