...
# Detect → install Argos pairs → translate all candidates → repartition detail → rebuild catalog → verify.
set -euo pipefail
...
echo "== rebuild claude_index + alpha_catalog + pantry hay from recipe_detail =="
"$PY" scripts/rebuild_catalog_from_detail.py
